...
function fillScene() {
  ...
  var glossyFloor = new THREE.Mesh(
    new THREE.PlaneGeometry(2000, 2000, 32),
    glossyMaterial
  )
  ...
  var wall = new THREE.Mesh(
    new THREE.PlaneGeometry(2000, 1000, 32),
    wallMaterial
  )
...
function drawMachineBody() {
  ...
  // texture
  var bodyTexture = new THREE.ImageUtils.loadTexture(
    'textures/Christmas_wallpapers_in_red_2_s.jpg'
  )
  ...
  coinSlot = new THREE.Mesh(
    new THREE.BoxGeometry(3, 23, 2),
    glowMaterial_silver
  )
  ...
  consoleTable = new THREE.Mesh(
    new THREE.BoxGeometry(100, 20, 50),
    bodyMaterial
  )
  ...
  stick = new THREE.Mesh(
    new THREE.CylinderGeometry(4, 4, 23, 16),
    bodyMaterial_yellow
  )
  ...
  ring = new THREE.Mesh(
    new THREE.CylinderGeometry(11.5, 11.5, 3, 32),
    bodyMaterial
  )
...
function drawClaw() {
  ...
  wire = new THREE.Mesh(
    new THREE.CylinderGeometry(0.7, 0.7, 200, 32),
    bodyMaterial
  )
  wireObject.add(wire)
  ...
  clawLeg = new THREE.Mesh(
    new THREE.CylinderGeometry(2.5, 2.5, 23, 16),
    clawLegMaterial_orange
  )
  legPartsG1.add(clawLeg)

  clawJoint = new THREE.Mesh(
  ...
  clawLeg = new THREE.Mesh(
    new THREE.CylinderGeometry(3, 0.5, 23, 16),
    clawLegMaterial_orange
  )
...
function keyState() {
  ...
  if (keyboard.down('space')) clawMovingDown = true

  if (keyboard.up('space')) buttonG.position.y = 353.5
  ...
  if (keyboard.pressed('left')) {
    if (bottomStructG.position.x < 100) bottomStructG.translateX(1)
  }
  if (keyboard.down('left')) joystickG.rotateZ(-Math.PI / 6)

  if (keyboard.up('left')) joystickG.rotateZ(+Math.PI / 6)

  //-------------------right--------------------//
  if (keyboard.pressed('right')) {
    if (bottomStructG.position.x > -100) bottomStructG.translateX(-1)
  }
  if (keyboard.down('right')) joystickG.rotateZ(+Math.PI / 6)
  if (keyboard.up('right')) joystickG.rotateZ(-Math.PI / 6)

  //---------------------up---------------------//
  if (keyboard.pressed('up')) {
    if (frameStructG.position.z < 115) frameStructG.translateZ(1)
  }
  if (keyboard.down('up')) joystickG.rotateX(Math.PI / 6)
  if (keyboard.up('up')) joystickG.rotateX(-Math.PI / 6)

  //--------------------down--------------------//
  if (keyboard.pressed('down')) {
    if (frameStructG.position.z > -115) frameStructG.translateZ(-1)
  }
  if (keyboard.down('down')) joystickG.rotateX(-Math.PI / 6)
  if (keyboard.up('down')) joystickG.rotateX(Math.PI / 6)
  //--------------------------------------------//
}
